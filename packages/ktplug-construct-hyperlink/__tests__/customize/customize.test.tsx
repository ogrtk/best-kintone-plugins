...
describe("customize.tsx", () => {
  ...
  describe("イベントハンドラー", () => {
    ...
    it("登録・更新画面表示時にフィールドが存在しない場合、エラーが発生する", async () => {
      ...
      expect(() => eventHandler(mockEvent)).toThrow("指定の項目がありません:link_field");
      expect(alertSpy).toHaveBeenCalledWith("指定の項目がありません:link_field");
    });
    ...
    it("保存時にurlPartsFieldCodeが存在しない場合、エラーが発生する", async () => {
      ...
      // イベントハンドラーを実行してエラーが発生することを確認
      expect(() => eventHandler(mockEvent)).toThrow("指定の項目がありません:id_field");
    ...
    it("詳細画面表示時にリンクが設定される", async () => {
      ...
      const getFieldElementSpy = vi.spyOn(kintone.app.record, "getFieldElement");
    ...
    it("一覧画面表示時にlinkElementsが存在しない場合、console.warnが呼ばれる", async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith("指定の項目がありません:link_field");
  ...
  describe("ユーティリティ関数", () => {
    it("setLinkElement - スタイルとイベントリスナーが設定される", async () => {
      ...
      if (clickHandler) {
        clickHandler();
        expect(windowOpenSpy).toHaveBeenCalledWith("https://example.com/test", "_blank");
      }

      // マウスオーバーイベントをシミュレート
      const mouseoverHandler = addEventListenerSpy.mock.calls.find(call => call[0] === "mouseover")?.[1] as Function;
      if (mouseoverHandler) {
        mouseoverHandler();
